...
struct XYZ /* Three-component vector */
{
	...
	// Operators handling colour vectors (RGB):
...
// There. The previous part was just basic vector and matrix mathematics.
...
// has a location and an RGB color.
	{ {{-28,-14,  3}}, {{ .4,.51, .9}} },
	{ {{-29,-29,-29}}, {{.95, .1, .1}} },
	{ {{ 14, 29,-14}}, {{ .8, .8, .8}} },
	{ {{ 29, 29, 29}}, {{1.0,1.0,1.0}} },
	{ {{ 28,  0, 29}}, {{ .5, .6, .1}} }
};
...
const unsigned
	...
	MAXTRACE = 2; // Maximum trace level
...
void RayTrace(XYZ& resultcolor, const XYZ& eye, const XYZ& dir, int k)
{
	...
	if (HitType != -1)
	{
		...
		// is an obstacle begind it. This is called "diffuse light".
...
void InitDither()
{
	...
	// Create bayer dithering matrix, adjusted for candidate cound
	for (unsigned y=0; y<8; y++)
		for (unsigned x=0; x<8; x++)
		{
			unsigned i = x ^ y, j;
			j = (x & 4)/4u + (x &2)*2u + (x & 1)*16u;
			i = (i & 4)/4u + (i &2)*2u + (i & 1)*32u;
...
int main(int argc, char **argv) {
	...
	XYZ camlook = {{0,0,0}};
	XYZ camlookdelta = {{-0.001, .005, .004}};
	...
	for (unsigned frameno = 0; frameno < 9300; frameno++)
	{
		...
		// Determine the contrast ration for this frame's pixels
	...
	#pragma omp parallel for
		for (unsigned y = 0; y < H; y++)
		{
			for (unsigned x = 0; x < W; x++)
			{
				XYZ camray = {{ x / double(W) - 0.5,
								y / double(H) - 0.5,
								zoom }};
				camray.d[0] *= 4.0/3; // Aspect ration correction
				camray.Normalize();
				XYZ campix;
				RayTrace(campix, campos, camray, MAXTRACE);
				campix *= 0.5; // adjust brightness...
			#pragma omp critical
			{
				...
				*/
				...
				//drawPixel(x, y, XYZ color);

				// Draw pixel (use BIOS)
				/* _AL = candlist[Dither8x8[x & 7][y & 7]]; */
			}
		}
		...
		else
		{
			...
			else if (zoom > 40) { if (zoomdelta < 0.99) zoomdelta += 0.001; }
			zoom += zoomdelta;
