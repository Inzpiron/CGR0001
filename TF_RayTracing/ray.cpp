...
int main(int argc, char **argv) // <<<<----- boa pratica
{
	...
	double zoom = 7.0, zoomdelta = 0.99;
            ...
            for(unsigned x=0; x<W; ++x)
            {
              ...
              #pragma omp critical
              {
				...
				// TODO Draw pixel
				{
					...
					/*
					pixels[((y*W) + x) + 0] = char(campix.d[0]*255); // Red
					pixels[((y*W) + x) + 1] = char(campix.d[1]*255); // Green
					pixels[((y*W) + x) + 2] = char(campix.d[2]*255); // Blue
					pixels[((y*W) + x) + 3] = 255; // Alpha
					*/
